...
export const ReportItemModal = ({ isOpen, onClose, itemId, itemName }: ReportItemModalProps) => {
  const { user } = useAuth();
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          message: `Item Report - ${reportType}: ${message.trim()}`,
          action_taken: `Reported item: ${itemName} (ID: ${itemId})`,
        });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Report Item: {itemName}</DialogTitle>
        </DialogHeader>
