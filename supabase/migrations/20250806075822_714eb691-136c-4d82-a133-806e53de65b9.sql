-- Fix deletion order to handle foreign key constraints properly
...
BEGIN
    ...
    -- Markets and vendors that reference submissions (delete these BEFORE submissions)
    DELETE FROM markets WHERE submission_id IN (SELECT id FROM submissions WHERE created_by = current_user_id OR reviewed_by = current_user_id);
    DELETE FROM vendors WHERE submission_id IN (SELECT id FROM submissions WHERE created_by = current_user_id OR reviewed_by = current_user_id);
    
    -- Now delete submissions and related data
